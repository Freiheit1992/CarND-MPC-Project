...
public:
  ...
  void operator()(ADvector &fg, const ADvector &vars)
  {
    ...
    for (int t = 1; t < N; ++t)
    {
      ...
      AD<double> ref_psi_tan = 0;
      for (int i = 1; i < coeffs.size(); i++)
      {
        f0 += CppAD::pow(x0, i) * coeffs[i];
        ref_psi_tan += i * coeffs[i] * CppAD::pow(x0, i - 1);
      }
      AD<double> psides0 = CppAD::atan(ref_psi_tan);
...
std::vector<double> MPC::Solve(const VectorXd &state, const VectorXd &coeffs)
{
  ...
  // ret.push_back(solution.x[psi_start + 1]);
  // ret.push_back(solution.x[v_start + 1]);
  // ret.push_back(solution.x[cte_start + 1]);
  // ret.push_back(solution.x[epsi_start + 1]);
